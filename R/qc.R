...
for (s in unique(d$sample))
{
    for (ce in unique(d[sample == s,]$cell))
    {
        ...
        all <- ggdraw() + draw_plot(plt) + 
            ...
            draw_label(paste("Avg. binwidth:", format(binwidth, big.mark=",")), x=.4, y=.94, vjust=1, hjust=0, size=10) +
            draw_label(paste("Number bins:", format(num_bins, big.mark=",")), x=.4, y=.92, vjust=1, hjust=0, size=10) +
            draw_label(paste("Reads/bin:", reads_per_bin), x=.4, y=.90, vjust=1, hjust=0, size=10)
        print(all)
    }
}
dev.off()
