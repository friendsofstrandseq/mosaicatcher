...
if (length(args) < 2 || length(args) > 3) {
    ...
    message("")
    message("Generates one plot per chromosome listing all cells below another.")
    message("If an SV probability file is provided, segments are additionally")
    message("colored by their predicted SV state")
    stop()
}
...
for (CHROM in unique(counts[, chrom])) {
    ...
    while (i < n_cells) {
        ...
        # Add GT colors:
        if(!is.null(f_prob) && nrow(local_probs)>0) {
            plt <- plt + 
